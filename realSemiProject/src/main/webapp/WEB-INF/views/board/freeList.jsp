...
<div class="container w-800">
	...
	<div class="right">
		<a class="link" href="reviewList">후기게시판으로></a>
	</div>
	...
	<div class="row mv-30 page-navigator">
		<c:if test="${!vo.first}">
			<a href="freeList?${vo.prevQueryStringForMemberList}">
		...
		<c:forEach var="i" begin="${vo.begin}" end="${vo.end}" step="1">
			<c:choose>
				...
				<c:otherwise>
					<%-- 링크는 검색과 목록을 따로 구현 --%>
					<a href="freeList?${vo.getQueryStringForMemberList(i)}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${!vo.last}">
			<%-- 링크는 검색과 목록을 따로 구현 --%>
				<a href="freeList?${vo.nextQueryStringForMemberList}">
